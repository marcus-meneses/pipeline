...
class Cog {
    ...
    error(message, status){
        return {error: { message: message || 'Unexpected behavior!', status: status || 500 }};
    }
...
class Method {
    ...
    startCountdown(methodIdentifier){
        global.timeouts[methodIdentifier] = setTimeout(()=>{
            //console.log(`${methodIdentifier} timed out`);
    ...
    resetCountdown(methodIdentifier){
        if (global.timeouts[methodIdentifier]){
            //console.log(`${methodIdentifier} refreshed`);
    ...
    constructor(methodName, context, verb){
        ...
        if (!global.methods[methodName]){
            ...
            try {
                this.methodCode = fileSystem.readFileSync(`./methods/${methodName}.js`, 'utf8');
            } catch (error) {
                this.return = this.context.cog.error(`Method ${this.methodName} unknown`, 404);
            }
            
            global.methods[methodName] = new virtualMachine.Script(this.methodCode);
    ...
    async run(){  
        try {
            if (!global.methods[this.methodName]) return { error:{message:`Method ${this.methodName} unknown`, status:404 }};
            virtualMachine.createContext(this.context);
            await global.methods[this.methodName].runInContext(this.context,{timeout:1000});
            this.return = this.context.cog.returnData.__magic_empty? null : this.context.cog.returnData;
            if (this.return == null) 
                return this.context.cog.error(`Cannot ${this.verb} ${this.methodName}`, 404);
            return this.return;
        } catch (error) {
